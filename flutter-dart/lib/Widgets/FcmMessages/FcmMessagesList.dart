...
import 'FcmMessagesAdd.dart';
import 'FcmMessagesEdit.dart';
import 'FcmMessagesProvider.dart';


class FcmMessagesList extends StatefulWidget {
  const FcmMessagesList({super.key});

  @override
  State<FcmMessagesList> createState() => _FcmMessagesListState();
}

class _FcmMessagesListState extends State<FcmMessagesList> {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<FcmMessagesProvider>(context);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('FcmMessages Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, FcmMessagesAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<FcmMessage> data) {
  ...
  Widget _buildCard(FcmMessage item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
                  ...
                  onPressed: () {
                    Navigator.of(context).push(Utils().createRoute(
                        ...
                        FcmMessagesEdit(
                          ...
                          data: item,
                          body : item.body,recipients : item.recipients
                        )));
                  },
                ),
                IconButton(
