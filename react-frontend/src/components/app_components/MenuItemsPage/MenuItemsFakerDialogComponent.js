...
import menuItemsFakerFactory from "./menuItemsFakerFactory";
...
const MenuItemsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [userContextIds, setuserContextIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("profileMenu")
                        .find({ query: { $select: ["_id","user"] } })
                        .then((data) => {
                        setuserContextIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = menuItemsFakerFactory(props.user,fakerCount,userContextIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("menuItems").create(elem));
...
export default connect(mapState, mapDispatch)(MenuItemsFakerDialogComponent);
