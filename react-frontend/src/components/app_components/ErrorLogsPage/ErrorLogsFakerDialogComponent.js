...
import errorLogsFakerFactory from "./errorLogsFakerFactory";
...
const ErrorLogsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = errorLogsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("errorLogs").create(elem));
...
export default connect(mapState, mapDispatch)(ErrorLogsFakerDialogComponent);
