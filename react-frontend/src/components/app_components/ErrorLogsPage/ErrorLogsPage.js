...
import ErrorLogsEditDialogComponent from "./ErrorLogsEditDialogComponent";
import ErrorLogsCreateDialogComponent from "./ErrorLogsCreateDialogComponent";
import ErrorLogsFakerDialogComponent from "./ErrorLogsFakerDialogComponent";
import ErrorLogsSeederDialogComponent from "./ErrorLogsSeederDialogComponent";
...
const ErrorLogsPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("errorLogs");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "ErrorLogs", type: "error", message: error.message || "Failed get ErrorLogs" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("errorLogs").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "ErrorLogs", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("errorLogs").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="errorLogs-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="errorLogs-datatable">
                    <ErrorLogsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ErrorLogsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ErrorLogsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ErrorLogsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ErrorLogsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
