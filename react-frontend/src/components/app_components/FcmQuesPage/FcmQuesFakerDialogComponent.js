...
import fcmQuesFakerFactory from "./fcmQuesFakerFactory";
...
const FcmQuesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = fcmQuesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("fcmQues").create(elem));
...
export default connect(mapState, mapDispatch)(FcmQuesFakerDialogComponent);
