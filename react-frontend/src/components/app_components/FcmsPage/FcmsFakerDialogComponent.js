...
import fcmsFakerFactory from "./fcmsFakerFactory";
...
const FcmsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = fcmsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("fcms").create(elem));
...
export default connect(mapState, mapDispatch)(FcmsFakerDialogComponent);
