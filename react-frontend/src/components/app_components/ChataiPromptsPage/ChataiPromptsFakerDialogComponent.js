...
import chataiPromptsFakerFactory from "./chataiPromptsFakerFactory";
...
const ChataiPromptsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [chatAiEnablerIds, setchatAiEnablerIds] = useState();
const [chatAiConfigIds, setchatAiConfigIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("chataiEnabler")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setchatAiEnablerIds(data.data);
                        });
client.service("chataiConfig")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setchatAiConfigIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = chataiPromptsFakerFactory(props.user,fakerCount,chatAiEnablerIds,chatAiConfigIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("chataiPrompts").create(elem));
...
export default connect(mapState, mapDispatch)(ChataiPromptsFakerDialogComponent);
