...
import ChataiPromptsDatatable from "./ChataiPromptsDataTable";
...
import ChataiPromptsFakerDialogComponent from "./ChataiPromptsFakerDialogComponent";
import ChataiPromptsSeederDialogComponent from "./ChataiPromptsSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const ChataiPromptsPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("chataiPrompts");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , chatAiEnabler : urlParams.singleChataiEnablerId ,chatAiConfig : urlParams.singleChataiConfigId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
                ...
                select: ["name"],
                        ...
                        path : "chatAiEnabler",
                        service : "chatai_enabler",
                        select:["name"]
                    },{
                        path : "chatAiConfig",
                        service : "chatai_config",
                        select:["name"]
                    }
            ] }})
            .then((res) => {
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Chat Ai Prompts", type: "error", message: error.message || "Failed get Chat Ai Prompts" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("chataiPrompts").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Chat Ai Prompts", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("chataiPrompts").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Chat Ai Prompts",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/chataiPrompts/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="chataiPrompts-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="chataiPrompts-datatable">
                    <ChataiPromptsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ChataiPromptsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ChataiPromptsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ChataiPromptsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ChataiPromptsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ChataiPromptsPage);
