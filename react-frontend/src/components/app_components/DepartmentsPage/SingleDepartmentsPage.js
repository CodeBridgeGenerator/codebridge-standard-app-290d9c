  ...
  const [isHelpSidebarVisible, setHelpSidebarVisible] = useState(false);
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((res) => {
                set_entity(res || {});
                const companyIds = Array.isArray(res.companyIds)
            ? res.companyIds.map((elem) => ({ _id: elem._id, name: elem.name }))
            : res.companyIds
                ? [{ _id: res.companyIds._id, name: res.companyIds.name }]
                : [];
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Is default</label><p className="m-0" ><i id="isDefault" className={`pi ${_entity?.isDefault?"pi-check": "pi-times"}`}  ></i></p></div>
            <div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Company Ids</label>
                    {companyIds.map((elem) => (
    ...
    <div className="col-12 mt-2">
        <TabView>
                    ...
                    <TabPanel header="Sections" leftIcon="pi pi-building-columns mr-2">
                        <SectionsPage/>
                    </TabPanel>
                    ...
                    <TabPanel header="Department HOD" leftIcon="pi pi-building-columns mr-2">
                        <DepartmentHODPage/>
                    </TabPanel>
                    

                    <TabPanel header="Department HOS" leftIcon="pi pi-building-columns mr-2">
                        <DepartmentHOSPage/>
                    </TabPanel>
