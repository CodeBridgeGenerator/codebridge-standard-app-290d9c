...
const DepartmentsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            company: _entity?.company?._id,deptName: _entity?.deptName,code: _entity?.code,isDefault: _entity?.isDefault || false,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="deptName" className="w-full mb-3 p-inputtext-sm" value={_entity?.deptName} onChange={(e) => setValByKey("deptName", e.target.value)}  />
            </span>
            <small className="p-error">
