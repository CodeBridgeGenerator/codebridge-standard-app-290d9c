...
const SingleTemplatesPage = (props) => {
  ...
  const [isHelpSidebarVisible, setHelpSidebarVisible] = useState(false);
    ...
    useEffect(() => {
        ...
        client
            .service("templates")
            .get(urlParams.singleTemplatesId, { query: { $populate: [            {
                path: "createdBy",
                service: "users",
                select: ["name"],
              },{
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Body</label><p className="m-0 ml-3"  dangerouslySetInnerHTML={{__html: _entity?.body}}></p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Variables</label><p className="m-0 ml-3" >{_entity?.variables}</p></div>
