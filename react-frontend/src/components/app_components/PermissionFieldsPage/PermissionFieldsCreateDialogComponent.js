...
const PermissionFieldsCreateDialogComponent = (props) => {
    ...
    useEffect(() => {
        let init  = {onCreate: false,onUpdate: false,onDetail: false,onTable: false};
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            servicePermissionId: _entity?.servicePermissionId?._id,fieldName: _entity?.fieldName,onCreate: _entity?.onCreate || false,onUpdate: _entity?.onUpdate || false,onDetail: _entity?.onDetail || false,onTable: _entity?.onTable || false,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Permission Fields updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Permission Fields" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Permission Fields" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="permissionFields-create-dialog-component">
