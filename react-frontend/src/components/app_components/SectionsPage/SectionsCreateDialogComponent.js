...
const SectionsCreateDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [departmentId], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            departmentId: _entity?.departmentId?._id,name: _entity?.name,code: _entity?.code,isDefault: _entity?.isDefault || false,
        ...
        props.onCreateResult(eagerResult.data[0]);
            ...
            props.alert({ type: "error", title: "Create", message: "Failed to create in Sections" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Sections" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
