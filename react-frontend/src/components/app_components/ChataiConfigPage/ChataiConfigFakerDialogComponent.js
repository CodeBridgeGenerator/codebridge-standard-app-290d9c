...
import chataiConfigFakerFactory from "./chataiConfigFakerFactory";
...
const ChataiConfigFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = chataiConfigFakerFactory(props.user,fakerCount,chatAiEnablerIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("chataiConfig").create(elem));
...
export default connect(mapState, mapDispatch)(ChataiConfigFakerDialogComponent);
