...
import ChataiConfigDatatable from "./ChataiConfigDataTable";
import ChataiConfigEditDialogComponent from "./ChataiConfigEditDialogComponent";
import ChataiConfigCreateDialogComponent from "./ChataiConfigCreateDialogComponent";
import ChataiConfigFakerDialogComponent from "./ChataiConfigFakerDialogComponent";
import ChataiConfigSeederDialogComponent from "./ChataiConfigSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const ChataiConfigPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("chataiConfig");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , chatAiEnabler : urlParams.singleChataiEnablerId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Chat Ai Config", type: "error", message: error.message || "Failed get Chat Ai Config" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("chataiConfig").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Chat Ai Config", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("chataiConfig").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Chat Ai Config",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/chataiConfig/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="chataiConfig-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="chataiConfig-datatable">
                    <ChataiConfigDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ChataiConfigEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ChataiConfigCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ChataiConfigFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ChataiConfigSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ChataiConfigPage);
