...
import superiorsFakerFactory from "./superiorsFakerFactory";
...
const SuperiorsFakerDialogComponent = (props) => {
...
client.service("staffinfo")
    ...
    const onRun = async () => {
        let fakeData = superiorsFakerFactory(props.user,fakerCount,superiorIds,subordinateIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("superiors").create(elem));
...
export default connect(mapState, mapDispatch)(SuperiorsFakerDialogComponent);
