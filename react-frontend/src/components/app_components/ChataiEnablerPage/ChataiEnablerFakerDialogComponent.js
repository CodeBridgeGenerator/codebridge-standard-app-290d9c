...
import chataiEnablerFakerFactory from "./chataiEnablerFakerFactory";
...
const ChataiEnablerFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = chataiEnablerFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("chataiEnabler").create(elem));
...
export default connect(mapState, mapDispatch)(ChataiEnablerFakerDialogComponent);
