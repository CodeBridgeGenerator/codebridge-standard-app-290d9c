...
import ChataiEnablerDatatable from "./ChataiEnablerDataTable";
import ChataiEnablerEditDialogComponent from "./ChataiEnablerEditDialogComponent";
import ChataiEnablerCreateDialogComponent from "./ChataiEnablerCreateDialogComponent";
import ChataiEnablerFakerDialogComponent from "./ChataiEnablerFakerDialogComponent";
import ChataiEnablerSeederDialogComponent from "./ChataiEnablerSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const ChataiEnablerPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("chataiEnabler");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Chat Ai Enabler", type: "error", message: error.message || "Failed get Chat Ai Enabler" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("chataiEnabler").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Chat Ai Enabler", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("chataiEnabler").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Chat Ai Enabler",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/chataiEnabler/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="chataiEnabler-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="chataiEnabler-datatable">
                    <ChataiEnablerDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ChataiEnablerEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ChataiEnablerCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ChataiEnablerFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ChataiEnablerSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ChataiEnablerPage);
