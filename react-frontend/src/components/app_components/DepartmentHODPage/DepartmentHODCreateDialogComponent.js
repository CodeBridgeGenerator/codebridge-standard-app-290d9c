...
const [Head, setHead] = useState([])
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            Department: _entity?.Department?._id,Head: _entity?.Head?._id,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Department Heads updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Department Heads" });
        }
        setLoading(false);
    };
...
useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setHead(res.data.map((e) => { return { name: e['fullname'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.debug({ error });
...
const HeadOptions = Head.map((elem) => ({ name: elem.name, value: elem.value }));
