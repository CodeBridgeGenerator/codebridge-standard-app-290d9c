...
const [Head, setHead] = useState([]);
    ...
    useEffect(() => {
        ...
        client
            ...
            .get(urlParams.singleDepartmentHODId, { query: { $populate: [            {
                path: "createdBy",
                service: "users",
                select: ["name"],
              },{
...
const Head = Array.isArray(res.Head)
        ...
        setHead(Head);
            ...
            .catch((error) => {
                console.log({ error });
                props.alert({ title: "DepartmentHOD", type: "error", message: error.message || "Failed get departmentHOD" });
            });
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Head</label>
                    {Head.map((elem) => (
