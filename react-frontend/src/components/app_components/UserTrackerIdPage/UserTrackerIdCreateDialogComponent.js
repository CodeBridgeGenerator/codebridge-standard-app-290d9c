...
const UserTrackerIdCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.pageName)) {
                error["pageName"] = `Page Name field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            pageName: _entity?.pageName,trackerCode: _entity?.trackerCode,userAgent: _entity?.userAgent,language: _entity?.language,timeZone: _entity?.timeZone,cookeisEnabled: _entity?.cookeisEnabled,doNotTrack: _entity?.doNotTrack,hardConcurrency: _entity?.hardConcurrency,marketCode: _entity?.marketCode,isLoggedIn: _entity?.isLoggedIn || false,userId: _entity?.userId?._id,
        ...
        const result = await client.service("userTrackerId").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info User Tracker updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in User Tracker" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create User Tracker" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="userTrackerId-create-dialog-component">
...
export default connect(mapState, mapDispatch)(UserTrackerIdCreateDialogComponent);
