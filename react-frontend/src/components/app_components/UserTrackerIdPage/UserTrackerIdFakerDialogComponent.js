...
import userTrackerIdFakerFactory from "./userTrackerIdFakerFactory";
...
const UserTrackerIdFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userTrackerIdFakerFactory(props.user,fakerCount,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("userTrackerId").create(elem));
...
export default connect(mapState, mapDispatch)(UserTrackerIdFakerDialogComponent);
