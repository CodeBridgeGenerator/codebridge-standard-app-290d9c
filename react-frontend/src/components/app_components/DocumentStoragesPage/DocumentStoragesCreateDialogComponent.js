...
const DocumentStoragesCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.eTag)) {
                error["eTag"] = `eTag field is required`;
                ret = false;
            }
  
            if (_.isEmpty(_entity?.versionId)) {
                error["versionId"] = `versionId field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,size: _entity?.size,path: _entity?.path,lastModifiedDate: _entity?.lastModifiedDate,lastModified: _entity?.lastModified,eTag: _entity?.eTag,versionId: _entity?.versionId,url: _entity?.url,tableId: _entity?.tableId,tableName: _entity?.tableName,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Document Storages created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Document Storages" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Document Storages" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="documentStorages-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="size" className="w-full mb-3 p-inputtext-sm" value={_entity?.size} onChange={(e) => setValByKey("size", e.value)}  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="lastModified" className="w-full mb-3 p-inputtext-sm" value={_entity?.lastModified} onChange={(e) => setValByKey("lastModified", e.value)}  />
            </span>
            <small className="p-error">
