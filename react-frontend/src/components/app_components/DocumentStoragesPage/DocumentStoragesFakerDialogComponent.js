...
import documentStoragesFakerFactory from "./documentStoragesFakerFactory";
...
const DocumentStoragesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = documentStoragesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("documentStorages").create(elem));
...
export default connect(mapState, mapDispatch)(DocumentStoragesFakerDialogComponent);
