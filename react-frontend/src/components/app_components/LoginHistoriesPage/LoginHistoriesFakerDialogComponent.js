...
import loginHistoriesFakerFactory from "./loginHistoriesFakerFactory";
...
const LoginHistoriesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = loginHistoriesFakerFactory(props.user,fakerCount,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("loginHistories").create(elem));
...
export default connect(mapState, mapDispatch)(LoginHistoriesFakerDialogComponent);
