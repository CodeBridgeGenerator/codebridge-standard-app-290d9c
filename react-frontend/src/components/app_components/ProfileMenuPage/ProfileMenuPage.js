...
const ProfileMenuPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("profileMenu")
            .find({ query: { $limit: 10000 , user : urlParams.singleUsersId ,roles : urlParams.singleRolesId ,positions : urlParams.singlePositionsId ,profiles : urlParams.singleProfilesId ,company : urlParams.singleCompaniesId ,branch : urlParams.singleBranchesId ,section : urlParams.singleSectionsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Profile Menu", type: "error", message: error.message || "Failed get Profile Menu" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            ...
            console.log({ error });
            props.alert({ title: "Profile Menu", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("profileMenu").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="profileMenu-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="profileMenu-datatable">
                    <ProfileMenuDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ProfileMenuEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ProfileMenuCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ProfileMenuFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ProfileMenuSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
