...
client.service("sections")
    ...
    const onRun = async () => {
        let fakeData = profileMenuFakerFactory(props.user,fakerCount,userIds,companyIds,branchIds,sectionIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("profileMenu").create(elem));
