...
const SingleMailQuesPage = (props) => {
  ...
  const [isHelpSidebarVisible, setHelpSidebarVisible] = useState(false);
    ...
    useEffect(() => {
        ...
        client
            .service("mailQues")
            .get(urlParams.singleMailQuesId, { query: { $populate: [            {
                path: "createdBy",
                service: "users",
                select: ["name"],
              },{
            ...
            .catch((error) => {
                console.log({ error });
                props.alert({ title: "MailQues", type: "error", message: error.message || "Failed get mailQues" });
            });
    }, [props,urlParams.singleMailQuesId]);


    const goBack = () => {
        navigate("/app/mailQues");
    };
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">From</label><p className="m-0 ml-3" >{_entity?.from}</p></div>
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">template Id</label><p className="m-0 ml-3" >{_entity?.templateId}</p></div>
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">JobId</label><p className="m-0 ml-3" >{Number(_entity?.jobId)}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">End</label><p className="m-0 ml-3" ><Calendar id="end" value={new Date(_entity?.end)} disabled={true} hourFormat="24"  /></p></div>
      ...
      <CommentsSection
        recordId={urlParams.singleMailQuesId}
        user={props.user}
        alert={props.alert}
