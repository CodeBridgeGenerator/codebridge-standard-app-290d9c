...
client.service("departments")
                        .find({ query: { $select: ["_id","deptName"] } })
                        .then((data) => {
...
client.service("userPhones")
    ...
    const onRun = async () => {
        let fakeData = profilesFakerFactory(props.user,fakerCount,userIdIds,departmentIds,sectionIds,roleIds,positionIds,managerIds,companyIds,branchIds,addressIds,phoneIds);
