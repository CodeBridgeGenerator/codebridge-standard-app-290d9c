...
const [phone, setPhone] = useState([]);
    ...
    useEffect(() => {
        ...
        client
            .service("profiles")
            .get(urlParams.singleProfilesId, { query: { $populate: [            {
                path: "createdBy",
                service: "users",
                select: ["name"],
              },{
                ...
                service: "users",
                select: ["name"],
              },"userId","department","section","role","position","manager","company","branch","address","phone"] }})
            .then((res) => {
                set_entity(res || {});
...
const address = Array.isArray(res.address)
            ? res.address.map((elem) => ({ _id: elem._id, street1: elem.street1 }))
            : res.address
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Bio</label><p className="m-0 ml-3" >{_entity?.bio}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Head of Department</label><p className="m-0" ><i id="hod" className={`pi ${_entity?.hod?"pi-check": "pi-times"}`}  ></i></p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Head of Section</label><p className="m-0" ><i id="hos" className={`pi ${_entity?.hos?"pi-check": "pi-times"}`}  ></i></p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Skills</label><p className="m-0 ml-3" ><Chip id="skills" label={_entity?.skills} /></p></div>
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Manager</label>
                    {manager.map((elem) => (
                        <Link key={elem._id} to={`/users/${elem._id}`}>
                        <div>
                  {" "}
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Phone</label>
                    {phone.map((elem) => (
    ...
    <div className="col-12 mt-2">
        <TabView>
                    ...
                    <TabPanel header="Profile Menu" leftIcon="pi pi-building-columns mr-2">
                        <ProfileMenuPage/>
                    </TabPanel>
                    

                    <TabPanel header="Permission Services" leftIcon="pi pi-building-columns mr-2">
