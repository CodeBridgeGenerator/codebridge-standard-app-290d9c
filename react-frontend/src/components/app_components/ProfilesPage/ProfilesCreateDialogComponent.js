...
const [phone, setPhone] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [userId,department,section,role,position,manager,company,branch,address,phone], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,userId: _entity?.userId?._id,image: _entity?.image,bio: _entity?.bio,department: _entity?.department?._id,hod: _entity?.hod || false,section: _entity?.section?._id,hos: _entity?.hos || false,role: _entity?.role?._id,position: _entity?.position?._id,manager: _entity?.manager?._id,company: _entity?.company?._id,branch: _entity?.branch?._id,skills: _entity?.skills,address: _entity?.address?._id,phone: _entity?.phone?._id,
        ...
        props.onCreateResult(eagerResult.data[0]);
            ...
            props.alert({ type: "error", title: "Create", message: "Failed to create in Profiles" });
        }
        setLoading(false);
    };
...
useEffect(() => {
                    ...
                    client
                        .service("userAddresses")
                        .find({ query: { $limit: 10000, $sort: { createdAt: -1 }, _id : urlParams.singleUserAddressesId } })
                        .then((res) => {
                            setAddress(res.data.map((e) => { return { name: e['street1'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.debug({ error });
                            props.alert({ title: "UserAddresses", type: "error", message: error.message || "Failed get userAddresses" });
                        });
                }, []);
...
useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setPhone(res.data.map((e) => { return { name: e['number'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.debug({ error });
...
const managerOptions = manager.map((elem) => ({ name: elem.name, value: elem.value }));
...
const phoneOptions = phone.map((elem) => ({ name: elem.name, value: elem.value }));

    return (
        <Dialog header="Create Profiles" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="profiles-create-dialog-component">
