...
const HelpSidebarContentsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            serviceName: _entity?.serviceName,content: _entity?.content,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Help Sidebar Contents created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Help Sidebar Contents" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Help Sidebar Contents" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
