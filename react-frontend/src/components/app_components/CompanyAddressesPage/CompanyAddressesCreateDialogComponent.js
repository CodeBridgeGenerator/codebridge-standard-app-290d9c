...
const CompanyAddressesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            companyId: _entity?.companyId?._id,Street1: _entity?.Street1,Street2: _entity?.Street2,Poscode: _entity?.Poscode,City: _entity?.City,State: _entity?.State,Province: _entity?.Province,Country: _entity?.Country,isDefault: _entity?.isDefault || false,
