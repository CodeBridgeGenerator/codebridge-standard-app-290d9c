  ...
  const [isHelpSidebarVisible, setHelpSidebarVisible] = useState(false);
    ...
    useEffect(() => {
        ...
        client
            ...
            .get(urlParams.singleCompanyPhonesId, { query: { $populate: [            {
                path: "createdBy",
                service: "users",
                select: ["name"],
              },{
        ...
        setCompanyId(companyId);
            ...
            .catch((error) => {
                console.log({ error });
                props.alert({ title: "CompanyPhones", type: "error", message: error.message || "Failed get companyPhones" });
            });
    }, [props,urlParams.singleCompanyPhonesId]);


    const goBack = () => {
        navigate("/app/companyPhones");
    };
  ...
  const copyPageLink = () => {
    ...
    return (
        ...
        <div className="col-12 flex flex-column align-items-center">
            ...
            <div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Country code</label><p id="countryCode" className="m-0" >{_entity?.countryCode}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Operator code</label><p id="operatorCode" className="m-0" >{_entity?.operatorCode}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Number</label><p id="number" className="m-0" >{_entity?.number}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Is Default</label><p className="m-0" ><i id="isDefault" className={`pi ${_entity?.isDefault?"pi-check": "pi-times"}`}  ></i></p></div>
      ...
      <CommentsSection
        recordId={urlParams.singleCompanyPhonesId}
        user={props.user}
        alert={props.alert}
