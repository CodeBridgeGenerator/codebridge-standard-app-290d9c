...
const UserInvitesPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("userInvites")
            .find({ query: { $limit: 10000 , position : urlParams.singlePositionsId ,role : urlParams.singleRolesId ,company : urlParams.singleCompaniesId ,branch : urlParams.singleBranchesId ,department : urlParams.singleDepartmentsId ,section : urlParams.singleSectionsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "User Invites", type: "error", message: error.message || "Failed get User Invites" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            ...
            console.log({ error });
            props.alert({ title: "User Invites", type: "error", message: error.message || "Failed delete record" });
        }
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="userInvites-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="userInvites-datatable">
                    <UserInvitesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <UserInvitesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
