...
client.service("branches")
                        .find({ query: { $select: ["_id","companyId"] } })
                        .then((data) => {
...
client.service("sections")
    ...
    const onRun = async () => {
        let fakeData = userInvitesFakerFactory(props.user,fakerCount,positionIds,roleIds,companyIds,branchIds,departmentIds,sectionIds);
