...
const [supervisor, setSupervisor] = useState([])
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            empNo: _entity?.empNo,name: _entity?.name,fullname: _entity?.fullname,company: _entity?.company?._id,department: _entity?.department?._id,section: _entity?.section?._id,position: _entity?.position?._id,supervisor: _entity?.supervisor?._id,dateJoined: _entity?.dateJoined,dateTerminated: _entity?.dateTerminated,resigned: _entity?.resigned,empGroup: _entity?.empGroup,empCode: _entity?.empCode,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Employees updated successfully" });
...
useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setSupervisor(res.data.map((e) => { return { name: e['name'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.debug({ error });
...
const supervisorOptions = supervisor.map((elem) => ({ name: elem.name, value: elem.value }));
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="dateJoined"  value={_entity?.dateJoined ? new Date(_entity?.dateJoined) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("dateJoined", new Date(e.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="dateTerminated"  value={_entity?.dateTerminated ? new Date(_entity?.dateTerminated) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("dateTerminated", new Date(e.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
