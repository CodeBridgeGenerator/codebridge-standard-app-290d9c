...
const sectionHead = Array.isArray(res.sectionHead)
            ? res.sectionHead.map((elem) => ({ _id: elem._id, fullname: elem.fullname }))
            : res.sectionHead
                ? [{ _id: res.sectionHead._id, fullname: res.sectionHead.fullname }]
                : [];
        setSectionHead(sectionHead);
            })
            .catch((error) => {
                console.log({ error });
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Section Head</label>
                    {sectionHead.map((elem) => (
