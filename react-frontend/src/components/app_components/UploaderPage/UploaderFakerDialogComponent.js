...
import uploaderFakerFactory from "./uploaderFakerFactory";
...
const UploaderFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = uploaderFakerFactory(props.user,fakerCount,userIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("uploader").create(elem));
...
export default connect(mapState, mapDispatch)(UploaderFakerDialogComponent);
