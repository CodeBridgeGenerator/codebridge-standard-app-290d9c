...
const PositionsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            roleId: _entity?.roleId?._id,name: _entity?.name,description: _entity?.description,abbr: _entity?.abbr,isDefault: _entity?.isDefault || false,
        ...
        props.onCreateResult(eagerResult.data[0]);
            ...
            props.alert({ type: "error", title: "Create", message: "Failed to create in Positions" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Positions" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
