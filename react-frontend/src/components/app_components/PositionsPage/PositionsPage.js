...
const PositionsPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("positions")
            .find({ query: { $limit: 10000 , roleId : urlParams.singleRolesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            ...
            console.log({ error });
            props.alert({ title: "Positions", type: "error", message: error.message || "Failed delete record" });
        }
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="positions-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="positions-datatable">
                    <PositionsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
