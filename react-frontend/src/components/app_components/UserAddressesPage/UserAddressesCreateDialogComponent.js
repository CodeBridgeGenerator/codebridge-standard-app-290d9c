...
const UserAddressesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            userId: _entity?.userId?._id,street1: _entity?.street1,street2: _entity?.street2,postalCode: _entity?.postalCode,city: _entity?.city,state: _entity?.state,province: _entity?.province,country: _entity?.country,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info User Addresses updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in User Addresses" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create User Addresses" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="userAddresses-create-dialog-component">
