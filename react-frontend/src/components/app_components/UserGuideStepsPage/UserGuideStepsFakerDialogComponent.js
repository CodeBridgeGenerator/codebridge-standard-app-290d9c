...
import userGuideStepsFakerFactory from "./userGuideStepsFakerFactory";
...
const UserGuideStepsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userGuideStepsFakerFactory(props.user,fakerCount,userGuideIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("userGuideSteps").create(elem));
...
export default connect(mapState, mapDispatch)(UserGuideStepsFakerDialogComponent);
