...
import UserGuideStepsDatatable from "./UserGuideStepsDataTable";
import UserGuideStepsEditDialogComponent from "./UserGuideStepsEditDialogComponent";
import UserGuideStepsCreateDialogComponent from "./UserGuideStepsCreateDialogComponent";
import UserGuideStepsFakerDialogComponent from "./UserGuideStepsFakerDialogComponent";
import UserGuideStepsSeederDialogComponent from "./UserGuideStepsSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const UserGuideStepsPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("userGuideSteps");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "User Guide Steps", type: "error", message: error.message || "Failed get User Guide Steps" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("userGuideSteps").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "User Guide Steps", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("userGuideSteps").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "User Guide Steps",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/userGuideSteps/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>User Guide Steps </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="userGuideSteps-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="userGuideSteps-datatable">
                    <UserGuideStepsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <UserGuideStepsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <UserGuideStepsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <UserGuideStepsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <UserGuideStepsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(UserGuideStepsPage);
