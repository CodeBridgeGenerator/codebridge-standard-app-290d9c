...
const FcmMessagesEditDialogComponent = (props) => {
...
from: _entity?.from?._id,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info fcmMessages updated successfully" });
...
const fromOptions = from.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit Fcm Messages" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="fcmMessages-edit-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <MultiSelect id="recipients" value={_entity?.recipients?.map((i) =>i._id)} options={recipientsOptions} optionLabel="name" optionValue="value" onChange={(e) => setValByKey("recipients", e.value)}  required  />
            </span>
            <small className="p-error">
...
export default connect(mapState, mapDispatch)(FcmMessagesEditDialogComponent);
