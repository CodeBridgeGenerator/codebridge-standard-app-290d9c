...
import fcmMessagesFakerFactory from "./fcmMessagesFakerFactory";
...
const FcmMessagesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = fcmMessagesFakerFactory(props.user,fakerCount,fromIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("fcmMessages").create(elem));
...
export default connect(mapState, mapDispatch)(FcmMessagesFakerDialogComponent);
