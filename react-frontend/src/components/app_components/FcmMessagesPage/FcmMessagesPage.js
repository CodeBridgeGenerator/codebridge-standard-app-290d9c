...
import FcmMessagesDatatable from "./FcmMessagesDataTable";
import FcmMessagesEditDialogComponent from "./FcmMessagesEditDialogComponent";
import FcmMessagesCreateDialogComponent from "./FcmMessagesCreateDialogComponent";
import FcmMessagesFakerDialogComponent from "./FcmMessagesFakerDialogComponent";
import FcmMessagesSeederDialogComponent from "./FcmMessagesSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const FcmMessagesPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("fcmMessages")
            .find({ query: { $limit: 10000 , recipients : urlParams.singleUsersId ,from : urlParams.singleUsersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Fcm Messages", type: "error", message: error.message || "Failed get Fcm Messages" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            ...
            console.log({ error });
            props.alert({ title: "Fcm Messages", type: "error", message: error.message || "Failed delete record" });
        }
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="fcmMessages-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="fcmMessages-datatable">
                    <FcmMessagesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <FcmMessagesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <FcmMessagesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <FcmMessagesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <FcmMessagesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(FcmMessagesPage);
